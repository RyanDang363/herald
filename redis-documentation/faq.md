...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"how-is-redis-different-from-other-key-value-stores","title":"How is Redis different from other key-value stores?"},{"id":"whats-the-redis-memory-footprint","title":"What's the Redis memory footprint?"},{"id":"why-does-redis-keep-its-entire-dataset-in-memory","title":"Why does Redis keep its entire dataset in memory?"},{"id":"can-you-use-redis-with-a-disk-based-database","title":"Can you use Redis with a disk-based database?"},{"id":"how-can-i-reduce-redis-overall-memory-usage","title":"How can I reduce Redis' overall memory usage?"},{"id":"what-happens-if-redis-runs-out-of-memory","title":"What happens if Redis runs out of memory?"},{"id":"background-saving-fails-with-a-fork-error-on-linux","title":"Background saving fails with a fork() error on Linux?"},{"id":"are-redis-on-disk-snapshots-atomic","title":"Are Redis on-disk snapshots atomic?"},{"id":"how-can-redis-use-multiple-cpus-or-cores","title":"How can Redis use multiple CPUs or cores?"},{"id":"what-is-the-maximum-number-of-keys-a-single-redis-instance-can-hold-what-is-the-maximum-number-of-elements-in-a-hash-list-set-and-sorted-set","title":"What is the maximum number of keys a single Redis instance can hold? What is the maximum number of elements in a Hash, List, Set, and Sorted Set?"},{"id":"why-does-my-replica-have-a-different-number-of-keys-its-master-instance","title":"Why does my replica have a different number of keys its master instance?"},{"id":"where-does-the-name-redis-come-from","title":"Where does the name \"Redis\" come from?"},{"id":"why-did-salvatore-sanfilippo-start-the-redis-project","title":"Why did Salvatore Sanfilippo start the Redis project?"},{"id":"how-is-redis-pronounced","title":"How is Redis pronounced?"}]}
...
[partitioning page](https://redis.io/docs/latest/operate/oss_and_stack/management/scaling) in this documentation for more info.
...
Beyond data modeling, there is more info in the [Memory Optimization page](https://redis.io/docs/latest/operate/oss_and_stack/management/optimization/memory-optimization).
...
is reached. See the [eviction policy docs](https://redis.io/docs/latest/develop/reference/eviction) for more information on this.
...
You can find more information about using multiple Redis instances in the [Partitioning page](https://redis.io/docs/latest/operate/oss_and_stack/management/scaling).
...
* These keys are still in the memory of the Redis primary, even if logically expired. They'll be considered non-existent, and their memory will be reclaimed later, either incrementally or explicitly on access. While these keys are not logically part of the dataset, they are accounted for in the [`INFO`](https://redis.io/docs/latest/commands/info) output and in the [`DBSIZE`](https://redis.io/docs/latest/commands/dbsize) command.
