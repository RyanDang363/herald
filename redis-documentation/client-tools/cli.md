...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"install-redis-cli","title":"Install redis-cli"},{"id":"command-line-usage","title":"Command line usage"},{"id":"string-quoting-and-escaping","title":"String quoting and escaping"},{"id":"host-port-password-and-database","title":"Host, port, password, and database"},{"id":"ssltls","title":"SSL/TLS"},{"id":"get-input-from-other-programs","title":"Get input from other programs"},{"id":"continuously-run-the-same-command","title":"Continuously run the same command"},{"id":"mass-insertion-of-data-using-redis-cli","title":"Mass insertion of data using redis-cli"},{"id":"csv-output","title":"CSV output"},{"id":"run-lua-scripts","title":"Run Lua scripts"},{"children":[{"id":"handle-connections-and-reconnections","title":"Handle connections and reconnections"},{"id":"editing-history-completion-and-hints","title":"Editing, history, completion and hints"},{"id":"preferences","title":"Preferences"},{"id":"run-the-same-command-n-times","title":"Run the same command N times"},{"id":"show-online-help-for-redis-commands","title":"Show online help for Redis commands"},{"id":"clear-the-terminal-screen","title":"Clear the terminal screen"}],"id":"interactive-mode","title":"Interactive mode"},{"children":[{"id":"continuous-stats-mode","title":"Continuous stats mode"}],"id":"special-modes-of-operation","title":"Special modes of operation"},{"children":[{"id":"big-keys","title":"Big keys"},{"id":"memory-usage","title":"Memory usage"},{"id":"combine-bigkeys-and-memkeys","title":"Combine --bigkeys and --memkeys"}],"id":"scan-for-big-keys-and-memory-usage","title":"Scan for big keys and memory usage"},{"id":"get-a-list-of-keys","title":"Get a list of keys"},{"id":"pubsub-mode","title":"Pub/sub mode"},{"id":"monitor-commands-executed-in-redis","title":"Monitor commands executed in Redis"},{"id":"monitor-the-latency-of-redis-instances","title":"Monitor the latency of Redis instances"},{"id":"remote-backups-of-rdb-files","title":"Remote backups of RDB files"},{"id":"replica-mode","title":"Replica mode"},{"id":"perform-an-lru-simulation","title":"Perform an LRU simulation"},{"id":"usage","title":"Usage"}]}
...
- [Install Redis Open Source](https://redis.io/docs/latest/operate/oss_and_stack/install/install-stack/). The `redis-cli` utility is installed as part of each installation method.
- [Build Redis from source](https://redis.io/docs/latest/operate/oss_and_stack/install/build-stack). Instead of building everything, you can just run the following command:

    `$ make redis-cli`.
    
    The `redis-cli` utility will be built in the `/path/to/redis-source/src` directory as `/path/to/redis-source/src/redis-cli`.

If you prefer not to install Redis, you can also run `redis-cli` in Docker. See the [Run `redis-cli` using Docker](https://redis.io/docs/latest/operate/oss_and_stack/install/install-stack/docker/#connect-with-redis-cli) page for instructions.
...
perform authentication saving the need of explicitly using the [`AUTH`](https://redis.io/docs/latest/commands/auth) command:
...
key content or [`INFO`](https://redis.io/docs/latest/commands/info) field output, or when we want to simulate some
...
worthwhile topic itself. Please refer to our [mass insertion guide](https://redis.io/docs/latest/develop/clients/patterns/bulk-loading).
...
of Lua scripting, available with Redis 3.2 onwards. For this feature, refer to the [Redis Lua debugger documentation](https://redis.io/docs/latest/develop/programmability/lua-debugging).
...
The Redis [`EVAL`](https://redis.io/docs/latest/commands/eval) command takes the list of keys the script uses, and the
other non key arguments, as different arrays. When calling [`EVAL`](https://redis.io/docs/latest/commands/eval) you
...
So `location:hastings:temp` will populate the [`KEYS`](https://redis.io/docs/latest/commands/keys) array, and `23` the `ARGV` array.
...
`redis-cli` provides online help for most Redis [commands](https://redis.io/docs/latest/commands), using the `HELP` command. The command can be used
in two forms:
...
For example in order to show help for the [`PFADD`](https://redis.io/docs/latest/commands/pfadd) command, use:
...
* Acting as a [Pub/Sub](https://redis.io/docs/latest/develop/pubsub) client to subscribe to channels.
* Monitoring the commands executed into a Redis instance.
* Checking the [latency](https://redis.io/docs/latest/operate/oss_and_stack/management/optimization/latency) of a Redis server in different ways.
...
* Simulating [LRU](https://redis.io/docs/latest/develop/reference/eviction) workloads for showing stats about keys hits.
...
The program uses the [`SCAN`](https://redis.io/docs/latest/commands/scan) command, so it can be executed against a busy
...
of second for each [`SCAN`](https://redis.io/docs/latest/commands/scan) command. 
...
patterns. This mode, like the `--bigkeys` option, uses the [`SCAN`](https://redis.io/docs/latest/commands/scan) command,
...
the [`SCAN`](https://redis.io/docs/latest/commands/scan) command with the `--pattern` option.
...
You can use `-i 0.01` to add a delay between calls to the [`SCAN`](https://redis.io/docs/latest/commands/scan) command.
...
the [`PUBLISH`](https://redis.io/docs/latest/commands/publish) command. Subscribing to channels in order to receive
...
but simply by using the [`SUBSCRIBE`](https://redis.io/docs/latest/commands/subscribe) or [`PSUBSCRIBE`](https://redis.io/docs/latest/commands/psubscribe) command, which are available in
...
once you use the [`MONITOR`](https://redis.io/docs/latest/commands/monitor) command. All commands received by the active Redis instance will be printed to the standard output:
...
option the CLI runs a loop where the [`PING`](https://redis.io/docs/latest/commands/ping) command is sent to the Redis
...
Redis is often used as a cache with [LRU eviction](https://redis.io/docs/latest/develop/reference/eviction).
...
- `-t <timeout>`: Server connection timeout in seconds (decimals allowed). Default timeout is 0, meaning no limit, depending on the OS.
...
- `-u <uri>`: Server URI on format redis://user:password@host:port/dbnum User, password and dbnum are optional. For authentication without a username, use username 'default'. For TLS, use the scheme 'rediss'.
- `-r <repeat>`: Execute specified command N times.
- `-i <interval>`: When -r is used, waits `<interval>` seconds per command. It is possible to specify sub-second times like -i 0.1. This interval is also used in --scan and --stat per cycle. and in --bigkeys, --memkeys, --keystats, and --hotkeys per 100 cycles.
...
- `--show-pushes <yn>`: Whether to print RESP3 PUSH messages.  Enabled by default when STDOUT is a tty but can be overridden with --show-pushes no.
...
- `--rdb <filename>`: Transfer an RDB dump from remote server to local file. Use filename of "-" to write to stdout.
- `--functions-rdb <filename>`: Like --rdb but only get the functions (not the keys) when getting the RDB dump file.
...
- `--keystats-samples <n>`: Sample Redis keys looking for keys memory size and length. And define number of key elements to sample (only for memory usage).
- `--cursor <n>`: Start the scan at the cursor `<n>` (usually after a Ctrl-C). Optionally used with --keystats and --keystats-samples.
- `--top <n>`: To display `<n>` top key sizes (default: 10). Optionally used with --keystats and --keystats-samples.
- `--hotkeys`: Sample Redis keys looking for hot keys. only works when maxmemory-policy is *lfu.
- `--scan`: List all keys using the SCAN command.
- `--pattern <pat>`: Keys pattern when using the --scan, --bigkeys, --memkeys, --keystats or --hotkeys options (default: *).
- `--count <count>`: Count option when using the --scan, --bigkeys, --memkeys, --keystats or --hotkeys (default: 10).
