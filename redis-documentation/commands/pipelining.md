...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"requestresponse-protocols-and-round-trip-time-rtt","title":"Request/Response protocols and round-trip time (RTT)"},{"id":"redis-pipelining","title":"Redis Pipelining"},{"id":"its-not-just-a-matter-of-rtt","title":"It's not just a matter of RTT"},{"id":"a-real-world-code-example","title":"A real world code example"},{"id":"pipelining-vs-scripting","title":"Pipelining vs Scripting"},{"id":"appendix-why-are-busy-loops-slow-even-on-the-loopback-interface","title":"Appendix: Why are busy loops slow even on the loopback interface?"}]}
...
![images/dev/reference/pipeline_iops.webp](https://redis.io/docs/latest/images/dev/reference/pipeline_iops.webp)
...
Using [Redis scripting](https://redis.io/docs/latest/commands/eval), available since Redis 2.6, a number of use cases for pipelining can be addressed more efficiently using scripts that perform a lot of the work needed at the server side.
...
Sometimes the application may also want to send [`EVAL`](https://redis.io/docs/latest/commands/eval) or [`EVALSHA`](https://redis.io/docs/latest/commands/evalsha) commands in a pipeline. 
This is entirely possible and Redis explicitly supports it with the [SCRIPT LOAD](https://redis.io/docs/latest/commands/script-load) command (it guarantees that [`EVALSHA`](https://redis.io/docs/latest/commands/evalsha) can be called without the risk of failing).
