...
**Current blocker:** _none — INTEGRATION WIRED on `evan/wire-memory-ehr-dashboard` (2026-06-21): `main.py` now selects backends via `make_store()`/`make_memory()`; the Orchestrator records/recalls via Iris (MEM-FLOW-001/002), Admissions enriches intake from the EHR with MRN dedupe (EHR-FLOW-001/002), and the dashboard reads live Redis snapshots + the `er:events` stream (DASH-SYS-002/003). **184 tests green + `ruff` clean** (incl. the data-driven replay layer — LLD §9.1). Verified live against Redis Cloud + Iris (RedisStore/IrisMemory selected; MRN-0007 history loaded into Redis; dashboard read back 4 er:events lines; Iris POST→201). All 11 arrows now OK._
**✅ ALL 3 EVENTS VERIFIED LIVE END-TO-END (2026-06-21, ASI:One → mailbox → Bureau, `USE_MOCK=false` Redis + Iris + ASI:One LLM):**

- **Intake** ("A new patient arrived with chest pain") → _"Admitted Jordan Lee. Triage ESI-2. Assigned bed-1 + Nurse Chen; paged Dr. Smith (cardiology). Replay captured → /replay/patient_intake-0001"_.
- **Oxygen** ("Bed 3's patient oxygen is dropping") → full **real-async-messaging** chain over `ctx.send`: `LowSupplyAlert (o2_1 45%)` → `unit_located o2_2` → `nurse2 accepts dispatch` → `oxygen_swap_complete o2_1→o2_2 at bed3 via nurse2` (6 replay snapshots; swap persisted: bed3 equipment=[o2_2]).
- **Summary** ("Show me what's happening in the ER") → read-only snapshot reflecting post-oxygen state: _"2 patients active, 1 bed occupied, 0 nurse(s) free. No critical alerts."_

Mailbox connect uses the standalone bootstrap (`er_twin/connect_orchestrator.py`) because the Agentverse Inspector rejects a Bureau. **Three bugs/gaps found + fixed live this run:** (1) ASI:One prepends an `@<agent-address>` mention → broke the exact-match intake lookup → "Unknown Patient"; fixed via `strip_agent_mention` + tolerant `lookup_mock_intake` (ORCH-CHAT-002). (2) Boot seed did not durably land beds/nurses in Redis → every intake `no_bed_available`; fixed via self-healing `ensure_seeded()` (seed → read-back → re-seed once + boot banner/WARNING) and a `find_available_bed` guard that no longer treats an empty bed record as free; new `scripts/seed_redis.py` for operator pre-flight. (3) **RCA root cause (confirmed):** `RedisStore.set`/`update` used `transaction=False`, so a connection reset/timeout mid-pipeline could `DEL` a key without the `HSET`/`SADD` (silent partial loss under the boot-time Almanac `TimeoutError`s) → fixed by `transaction=True` (MULTI/EXEC atomic writes); verified holding under a live timeout (state survived). **184 tests green + `ruff` clean.** **Remaining:** Phase P Pika pre-flight (Claude CLI + Pika auth; spends credits) + submission deliverables (ASI:One chat URL, Agentverse profile, demo video, Devpost)._
...
| R+ | Data-driven replay (LLD §9.1): snapshot timeline → `out/replay/{incident}.json`, `/replay/{incident}` playback (shared `floor.js`), keyframe capture (Playwright) → Pika `generate_keyframes_video`, gated `/library` | **Evan** | **complete** (REPLAY-SNAP/FRAME/KEY/LIB `[x]`, tested + screenshotted; `run_pika_keyframes.ps1` built — live Pika render pending operator pre-flight) |
...
| 6 | `RedisStore` (hashes + index sets + Streams), `make_store()` factory — **P6** | redis dev | **complete** (wired: `main.py` selects it via `make_store()`; verified live) |
| 7 | Iris Agent Memory (`IrisMemory`, `NoopMemory`, `make_memory()`), smoke script | redis dev + Evan | **complete + wired** (Orchestrator records/recalls — MEM-FLOW-001/002; live Iris POST→201) |
| 8 | EHR loader (`er_twin/ehr.py`), 20-patient fixture, `scripts/build_ehr.py`, `mrn` on intake | redis dev + Evan | **complete + wired** (intake EHR-enriched + MRN dedupe — EHR-FLOW-001/002; verified live) |
| Stretch | Dashboard (FastAPI + HTML) — **P7** | dashboard dev + Evan | **complete** (live Redis snapshot + `er:events` stream read — DASH-SYS-002/003; `DASHBOARD_SOURCE=redis`) |
