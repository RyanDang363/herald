...
├── ehr.py             # EHR loader: build_live_record(mrn, ...) merges the master EHR chart into an intake record
...
- **Idempotency:** state-mutating handlers are no-ops on duplicate input. A second `BedAssignRequest` for a patient already in that bed returns the existing assignment with `success=true` and writes nothing new; a duplicate intake for an already-active patient returns the existing `patient_id` rather than creating a second record (current dedupe key: `name` + `chief_complaint` among non-discharged patients). *With EHR integration (patient-ehr, pending), the intake dedupe key becomes **MRN** among non-discharged patients via `find_active_patient_by_mrn`, falling back to `name`+`chief_complaint` only when no MRN is present (unregistered walk-in).*
